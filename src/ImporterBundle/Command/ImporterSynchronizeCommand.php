...
class ImporterSynchronizeCommand extends ContainerAwareCommand
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        // Set the reader
        $this->csvReader = $this->getContainer()->get('importer_bundle.provider.csv');
        ...
        $output->writeln("");
        $output->writeln("");
        $output->writeln("Exportando productos de Base de datos a Prestashop");
    ...
    protected function importProducts(OutputInterface $output, EntityManager $em)
    {
        ...
        foreach ($allCSVRows as $csvRow) {
            if ($this->csvReader->processCSVRow($csvRow)) {
                $progress->advance();
    ...
    protected function exportProductsToPrestashop(
        ...
        foreach ($products as $product) {
            if(!empty($product->getRealProductId())){
                $productId = $product->getRealProductId();
            } else {
            ...
            switch ($actionType) {
                case CustomPrestashopWS::CREATE_ACTION:
                    $result = $webService->createProduct($product);
                    break;
                case CustomPrestashopWS::UPDATE_ACTION:
                    $result = $webService->updateProduct($product);
                    break;
